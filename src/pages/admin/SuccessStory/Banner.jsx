...
function Banner() {
  ...
  const [banner_heading, setBannerHeading] = useState('');
  const [banner_text, setBannerText] = useState('');
  const [button_text, setButtonText] = useState('');
  const [button_url, setButtonUrl] = useState('');
  const [banner_status, setBannerStatus] = useState('');
  const config = {};

// Function to reset all data
const resetAllData = () => {
  setImages(null); // Reset the images state to null
  setBannerHeading(null); // Reset banner_heading to null
  setBannerText(null); // Reset banner_text to null
  setButtonText(null); // Reset button_text to null
  setButtonUrl(null); // Reset button_url to null
  setBannerStatus(null); // Reset banner_status to null
}
  ...
  const handleButtonUrlChange = (buttonURLData) => {
    setButtonUrl(buttonURLData);
  }

  const handleButtonTextChange = (buttonTextData) => {
    setButtonText(buttonTextData);
  }

  const handleStatusChange = (event) => {
    setBannerStatus(event.target.value === 'Publish');
  };
